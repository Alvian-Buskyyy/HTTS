...
const DashboardLayout = ({ children, title, role = 'USER' }) => {
  ...
  useEffect(() => {
    ...
    if (userData) {
      ...
      setUser(parsedUser);
      // If no role prop was provided, use the role from user data
      if (role === 'USER' && parsedUser.role) {
      ...
      <div className="bg-white shadow-md fixed w-full z-50">
        <div className="flex justify-between items-center p-4">
          <div className="flex items-center">
            <button className="md:hidden mr-4 text-gray-500" onClick={() => setSidebarOpen(!sidebarOpen)}>
              <i className="fas fa-bars"></i>
            </button>
            <div className="flex items-center">
              <i className="fas fa-shield-halved text-primary text-2xl mr-2"></i>
              <span className="text-lg font-bold text-primary hidden md:inline">Halalan Thoyyiban</span>
            </div>
          </div>
      ...
      <div className={`bg-white w-64 shadow-lg fixed md:static inset-y-0 left-0 transform z-40 transition duration-300 mt-16 h-[calc(100%-4rem)] ${sidebarOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
        <nav className="p-4">
          <ul className="space-y-2">
            <li>
              <Link to={`/${userRole.toLowerCase()}/dashboard`} className="flex items-center p-2 rounded-lg text-gray-800 hover:bg-primaryLight hover:text-primary">
              ...
              <>
                ...
                <li>
                  <Link 
                    ...
                    onClick={() => {
                      // Set auth flag dan hapus flag dashboard untuk tampilan default
            ...
            <li>
              <Link to={`/${userRole.toLowerCase()}/profil`} className="flex items-center p-2 rounded-lg text-gray-800 hover:bg-primaryLight hover:text-primary">
                <i className="fas fa-user mr-3"></i>
                <span>Profil</span>
              </Link>
            </li>
            <li>
              <Link to={`/${userRole.toLowerCase()}/pengaturan`} className="flex items-center p-2 rounded-lg text-gray-800 hover:bg-primaryLight hover:text-primary">
