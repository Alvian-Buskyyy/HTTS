...
// Mengadopsi struktur pemeriksaan kesehatan dari PeternakHealth
const JagalHealth = () => {
  ...
  useEffect(() => {
    ...
    // load default item sehat
    (async () => {
      try {
        const token = localStorage.getItem('token');
        const headers = token ? { Authorization: `Bearer ${token}` } : {};
        const r = await fetch(`${API_BASE}/itemSehat`, { headers });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!ensureItemId) {
        ...
        if (createRes.ok) {
          const it = await createRes.json(); ensureItemId = it.id; setItemSehatId(it.id);
        }
      }
              ...
              <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="md:col-span-2 space-y-5">
                  ...
                  <div>
                    ...
                    <input list="daftar-dokter-jagal" type="text" value={formData.dokter} onChange={e=>setFormData({...formData, dokter:e.target.value})} className="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary" placeholder="Nama dokter hewan" required />
                    <datalist id="daftar-dokter-jagal">
