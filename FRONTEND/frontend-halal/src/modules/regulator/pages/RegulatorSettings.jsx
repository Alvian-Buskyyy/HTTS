...
const RegulatorSettings = () => {
  ...
  useEffect(() => {
    try {
      const saved = JSON.parse(localStorage.getItem('regulatorSettings') || '{}');
  ...
  const saveSettings = (section) => {
    const existing = JSON.parse(localStorage.getItem('regulatorSettings') || '{}');
    const payload = {
    ...
    localStorage.setItem('regulatorSettings', JSON.stringify(payload));
    alert('Pengaturan disimpan');
  };

  const exportData = () => {
    const exportObj = {
      settings: JSON.parse(localStorage.getItem('regulatorSettings') || '{}'),
    };
    const blob = new Blob([JSON.stringify(exportObj, null, 2)], { type: 'application/json' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `backup_regulator_${new Date().toISOString().slice(0,10)}.json`;
    a.click();
    URL.revokeObjectURL(url);
  };

  const clearLocalData = () => {
    if (!confirm('Hapus data lokal pengaturan regulator? Tindakan ini tidak bisa dibatalkan.')) return;
    localStorage.removeItem('regulatorSettings');
  ...
  return (
    <DashboardLayout title="Pengaturan" role="REGULATOR" customSidebar={<RegulatorSidebar /> }>
        ...
        <main className="md:col-span-3">
          ...
          {activeTab === 'account' && (
            <section className="bg-white rounded-lg border border-gray-100 p-5">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm text-gray-600 mb-1">Nama Instansi</label>
                  <input className="w-full border border-gray-300 rounded-md px-3 py-2" value={account.institutionName} onChange={e=>setAccount({...account, institutionName: e.target.value})} />
                </div>
                <div>
                  <label className="block text-sm text-gray-600 mb-1">Nama Pejabat</label>
                  <input className="w-full border border-gray-300 rounded-md px-3 py-2" value={account.officerName} onChange={e=>setAccount({...account, officerName: e.target.value})} />
                </div>
                <div>
            ...
            <section className="bg-white rounded-lg border border-gray-100 p-5">
              ...
              <div className="space-y-3">
                {[
                  ...
                  { key:'healthAlerts', label:'Peringatan Kepatuhan'},
                  { key:'transactionUpdates', label:'Update Sistem'},
          ...
          {activeTab === 'privacy' && (
            <section className="bg-white rounded-lg border border-gray-100 p-5">
              ...
              <div className="space-y-3">
                {[
                  { key:'publicProfile', label:'Tampilkan profil institusi secara publik'},
                  { key:'shareContact', label:'Bagikan kontak ke entitas terverifikasi'},
            ...
            <section className="bg-white rounded-lg border border-gray-100 p-5">
              ...
              <div className="space-y-4">
                <label className="flex items-center justify-between p-3 border rounded-md">
                  <span className="text-gray-700">Aktifkan 2FA (kode OTP saat aksi penting)</span>
          ...
          {activeTab === 'data' && (
            <section className="bg-white rounded-lg border border-gray-100 p-5">
              ...
              <div className="flex flex-wrap gap-3">
                ...
                <button onClick={clearLocalData} className="px-4 py-2 rounded-md border border-red-300 text-red-600 hover:bg-red-50"><i className="fas fa-trash mr-2"></i>Clear Local Data</button>
              </div>
              <p className="text-xs text-gray-500 mt-3">Export akan menyertakan pengaturan regulator yang tersimpan di perangkat ini.</p>
            </section>
          )}
        </main>
      </div>
