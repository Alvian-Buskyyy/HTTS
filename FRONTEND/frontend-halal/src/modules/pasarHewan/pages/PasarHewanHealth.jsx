...
const PasarHewanHealth = () => {
  ...
  // Sinkronisasi data: ambil sapi milik Pasar Hewan dari backend, bangun pending dan riwayat berbasis CID; fallback ke localStorage jika backend kosong
  useEffect(() => {
    (async () => {
      try {
        ...
        // Ambil userId dari localStorage user
        const userStr = localStorage.getItem('user');
        const user = userStr ? JSON.parse(userStr) : {};
        const userId = user?.id;

        // Resolusi pasarHewanId via endpoint user-based, fallback ke local data
        let resolvedPasarHewanId = null;
        if (userId) {
          try {
            const phRes = await fetch(`${API_BASE}/pasarHewan/user/${userId}`, { method: 'GET', headers });
            if (phRes.ok) {
              const phData = await phRes.json();
              resolvedPasarHewanId = phData?.id || phData?.data?.id || phData?.data?.pasarHewan?.id || phData?.pasarHewanId || null;
            }
          } catch (err) {
            console.warn('Gagal resolve pasarHewan via user endpoint:', err);
          }
        }
        if (!resolvedPasarHewanId) {
          resolvedPasarHewanId = user?.pasarHewanId || user?.entityId || null;
        }
        if (!resolvedPasarHewanId) {
          setErrorMessage('Gagal memuat data kesehatan. Silakan refresh atau coba lagi nanti.');
        }

        // Ambil daftar sapi milik Pasar Hewan dari backend
        let sapiMapped = [];
        if (resolvedPasarHewanId) {
          const sapiRes = await fetch(`${API_BASE}/sapi/entity/PASAR_HEWAN/${resolvedPasarHewanId}`, { method: 'GET', headers });
          if (sapiRes.ok) {
            ...
            sapiMapped = (Array.isArray(sapiData) ? sapiData : sapiData.data || []).map(c => ({
              ...
              source: 'Asal Rantai Pasok',
              arrivalDate: c.tanggalLahir ? new Date(c.tanggalLahir).toLocaleDateString('id-ID') : '-'
            }));
          } else {
            setErrorMessage('Gagal memuat daftar sapi untuk pemeriksaan.');
          }
        }

        // Ambil riwayat pengecekan sehat; tandai sehat hanya jika boolean true DAN ada CID; isi detail dari IPFS
            ...
            verificationRecords = await Promise.all(relevantChecks.map(async (ch) => {
              ...
              if (ch.cid) {
                try {
                  ...
                  console.warn('Gagal ambil IPFS payload untuk CID:', ch.cid, err);
                }
              }
              return {
                id: ch.sapiId,
                type: sapi?.type || '-',
                source: sapi?.source || 'Asal Rantai Pasok',
                verifiedDate,
        ...
        // Pending: sapi yang belum terverifikasi sehat (tanpa CID). Fallback ke localStorage bila backend kosong
        let pending = [];
        if (sapiMapped.length) {
          ...
          pending = mappedWithStatus
            ...
            .map(s => ({ id: s.id, type: s.type, source: s.source, arrivalDate: s.arrivalDate, weight: s.weight }));
        } else {
          try {
        ...
        console.error('Gagal memuat data kesehatan pasar hewan:', e);
  ...
  const handleSubmit = async (e) => {
    ...
    // Pindahkan ke riwayat hanya jika CID tersedia; jika belum, tetap pending
    if (created?.cid) {
      const record = {
      ...
      setSuccessMessage(`Verifikasi kesehatan untuk ${record.id} berhasil dan CID tercatat.`);
    } else {
      setSuccessMessage(`Verifikasi tersimpan. Menunggu CID IPFS untuk ${selectedCattle.id}.`);
    }
  ...
  return (
    <DashboardLayout title="Pemeriksaan & Verifikasi Kesehatan" role="PASAR_HEWAN">
      <div className="mt-2">
        ...
        {errorMessage && (
          <div className="mb-6 p-4 rounded-md border border-red-400 bg-red-100 text-red-700 text-sm">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {verificationHistory.map((r) => (
                  <tr key={r.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-5 py-3 whitespace-nowrap text-sm text-gray-700 text-left align-middle">{r.detail.suhu || '-'}</td>
                    <td className="px-5 py-3 whitespace-nowrap text-sm text-gray-700 text-left align-middle">
                      {(() => {
                        const d = r.detail || {};
                        const list = [];
                        if (d.pmk) list.push('PMK');
                        if (d.antraks) list.push('Antraks');
                        if (d.brucellosis) list.push('Brucellosis');
                        if (d.parasit) list.push('Parasit');
                        return list.length ? list.join(', ') : '-';
                      })()}
                    </td>
                    <td className="px-5 py-3 whitespace-nowrap text-sm text-gray-700 text-left align-middle">{r.detail.catatan || '-'}</td>
                    ...
                    <td className="px-5 py-3 whitespace-nowrap text-sm text-left align-middle">
                      {r.cid ? (
                        ...
                        <span className="text-gray-400">Menunggu CID</span>
                      )}
                    </td>
                  </tr>
                ))}
                {verificationHistory.length === 0 && (
                  <tr>
                    <td colSpan={10} className="px-5 py-4 text-left text-sm text-gray-500">Belum ada pemeriksaan dilakukan.</td>
                  </tr>
                )}
              </tbody>
            </table>
