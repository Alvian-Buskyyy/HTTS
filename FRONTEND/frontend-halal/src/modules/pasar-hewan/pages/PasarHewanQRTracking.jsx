...
const PasarHewanQRTracking = () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const filtered = allSapi.filter(
          (s) => s.currentOwnerType === "PASAR_HEWAN" && s.currentOwnerId === userData.id
        );
        setMySapi(filtered);
      }
    } catch (error) {
  ...
  return (
    <DashboardLayout title="QR Code & Tracking" role="Pasar Hewan" customSidebar={<PasarHewanSidebar />}>
      <div className="space-y-6">
        <div className="bg-white rounded-lg shadow-md p-6 border border-gray-200">
          ...
          {loading ? (
            ...
            <p className="text-sm text-gray-500 text-center py-8">Tidak ada sapi di pasar hewan</p>
          ) : (
        ...
        {selectedSapi && <QRTracking type="sapi" data={selectedSapi} role="pasar-hewan" />}
      </div>
    </DashboardLayout>
  );
};

export default PasarHewanQRTracking;
