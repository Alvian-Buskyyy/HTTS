...
// Mengadopsi persis struktur JagalDashboard (statistik & tabel) untuk RPH
const loadingCSS = `
  @keyframes spin { 0% { transform: rotate(0deg); } 100% { transform: rotate(360deg); } }
  .loader { border-top-color: #60A5FA; animation: spin 1s linear infinite; }
`;

const RphDashboard = ({ initialSection = 'dashboard' }) => {
  ...
  const showSection = (sectionName) => {
    if (sectionName === 'transactions') {
      ...
      navigate('/rph/transaksi');
    } else if (sectionName === 'dashboard') {
      navigate('/rph/dashboard');
    } else if (sectionName === 'sapi') {
      navigate('/rph/sapi');
    } else if (sectionName === 'daftar-ternak') {
      navigate('/rph/sapi'); // belum ada halaman khusus pendaftaran
  ...
  return (
    <DashboardLayout title="Dasbor RPH" role="RPH" customSidebar={<RphSidebar />}>
            ...
            <div className="space-y-8">
              <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                ...
                <div className="flex gap-2 flex-wrap">
                  <Link to="/rph/sapi" className="px-4 py-2 bg-primary text-white rounded-md text-sm hover:bg-primaryDark flex items-center gap-2"><i className="fas fa-plus"></i> Tambah Ternak</Link>
                  <Link to="/rph/transaksi" onClick={() => localStorage.setItem('transactionAuth','true')} className="px-4 py-2 bg-blue-600 text-white rounded-md text-sm hover:bg-blue-700 flex items-center gap-2"><i className="fas fa-exchange-alt"></i> Transaksi</Link>
                </div>
              </div>
              ...
              <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <Link to="/rph/sapi" className="text-primary text-sm hover:underline flex items-center gap-1"><i className="fas fa-eye"></i> Lihat Semua</Link>
                </div>
                {cattleData.length === 0 ? (
                  <EmptyState icon="fas fa-cow" title="Belum Ada Data Ternak" desc="Tambahkan ternak pertama untuk mulai memantau kesehatan dan transaksi." action={<Link to="/rph/sapi" className="px-4 py-2 bg-primary text-white rounded-md hover:bg-primaryDark"><i className="fas fa-plus mr-1"></i> Tambah Ternak</Link>} />
                ) : (
                          ...
                          <tr key={c.id} className="hover:bg-gray-50">
                            ...
                            <td className="px-4 py-2 text-left w-20">
                              <div className="flex items-center gap-2">
                                <button onClick={()=>viewCattleDetail(c.id)} className="px-2 py-1 rounded border text-primary border-primary/30 hover:bg-primary/10 inline-flex items-center justify-center" title="Detail">
                ...
                <div className="xl:col-span-2 space-y-6">
                  <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
                    <div className="flex items-center justify-between mb-4">
                      ...
                      <Link to="/rph/aktivitas" className="text-primary text-xs hover:underline">Lihat Semua</Link>
                    </div>
                  ...
                  <div className="bg-white rounded-lg max-w-2xl w-full mx-4 overflow-hidden">
                    ...
                    <div className="p-6 text-left">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                        ...
                        <div>
                          <p className="text-sm text-gray-500">Kesehatan</p>
