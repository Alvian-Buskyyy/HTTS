...
// Mengadopsi JagalSettings dengan penyesuaian field khusus RPH (sertifikat halal, JULEHA)
const RphSettings = () => {
  const [activeTab, setActiveTab] = useState('account');
  const [account, setAccount] = useState({ facilityName:'', companyName:'', email:'', phone:'', address:'', halalCertificate:'', julehaName:'', julehaCertificate:'', supervisors:'', timezone:'Asia/Jakarta', language:'id' });
  const [notifications, setNotifications] = useState({ email:true, whatsapp:true, slaughterAlerts:true, distributionUpdates:true, verificationEvents:true });
  ...
  const [integration, setIntegration] = useState({ walletAddress:'' });

  useEffect(()=>{ try{ const saved = JSON.parse(localStorage.getItem('rphSettings')||'{}'); if(saved.account) setAccount(saved.account); if(saved.notifications) setNotifications(saved.notifications); if(saved.privacy) setPrivacy(saved.privacy); if(saved.security) setSecurity(saved.security); if(saved.integration) setIntegration(saved.integration);}catch{} },[]);

  const saveSettings = (section) => { const existing = JSON.parse(localStorage.getItem('rphSettings')||'{}'); const payload = { account: existing.account || account, notifications: existing.notifications || notifications, privacy: existing.privacy || privacy, security: existing.security || security, integration: existing.integration || integration }; if(section==='account') payload.account = account; if(section==='notifications') payload.notifications = notifications; if(section==='privacy') payload.privacy = privacy; if(section==='security') payload.security = security; if(section==='integration') payload.integration = integration; localStorage.setItem('rphSettings', JSON.stringify(payload)); alert('Pengaturan disimpan'); };

  const exportData = () => { const exportObj = { settings: JSON.parse(localStorage.getItem('rphSettings')||'{}'), cattle: JSON.parse(localStorage.getItem('cattleList')||'[]') }; const blob = new Blob([JSON.stringify(exportObj,null,2)], { type:'application/json'}); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = `backup_rph_${new Date().toISOString().slice(0,10)}.json`; a.click(); URL.revokeObjectURL(url); };
  const clearLocalData = () => { if(!confirm('Hapus data lokal RPH?')) return; localStorage.removeItem('rphSettings'); alert('Data lokal dihapus'); };
  ...
  return (
    <DashboardLayout title="Pengaturan" role="RPH" customSidebar={<RphSidebar /> }>
        ...
        <main className="md:col-span-3">
          {activeTab==='account' && (
            <section className="bg-white rounded-lg border border-gray-100 p-5 text-left">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div><label className="block text-sm text-gray-600 mb-1">Nama Fasilitas</label><input className="w-full border border-gray-300 rounded-md px-3 py-2" value={account.facilityName} onChange={e=>setAccount({...account, facilityName:e.target.value})} /></div>
                ...
                <div><label className="block text-sm text-gray-600 mb-1">Sertifikat Halal</label><input className="w-full border border-gray-300 rounded-md px-3 py-2" value={account.halalCertificate} onChange={e=>setAccount({...account, halalCertificate:e.target.value})} /></div>
                <div><label className="block text-sm text-gray-600 mb-1">Nama JULEHA</label><input className="w-full border border-gray-300 rounded-md px-3 py-2" value={account.julehaName} onChange={e=>setAccount({...account, julehaName:e.target.value})} /></div>
                <div><label className="block text-sm text-gray-600 mb-1">No Sertifikat JULEHA</label><input className="w-full border border-gray-300 rounded-md px-3 py-2" value={account.julehaCertificate} onChange={e=>setAccount({...account, julehaCertificate:e.target.value})} /></div>
                <div><label className="block text-sm text-gray-600 mb-1">Jumlah Penyelia</label><input className="w-full border border-gray-300 rounded-md px-3 py-2" value={account.supervisors} onChange={e=>setAccount({...account, supervisors:e.target.value})} /></div>
          ...
          {activeTab==='notifications' && (
            <section className="bg-white rounded-lg border border-gray-100 p-5 text-left">
              ...
              <div className="space-y-3">
                {[{key:'email',label:'Email'},{key:'whatsapp',label:'WhatsApp'},{key:'slaughterAlerts',label:'Peringatan Jadwal Pemotongan'},{key:'distributionUpdates',label:'Update Distribusi'},{key:'verificationEvents',label:'Event Verifikasi'}].map(i=> (
          ...
          {activeTab==='privacy' && (
            <section className="bg-white rounded-lg border border-gray-100 p-5 text-left">
              ...
              <div className="space-y-3">
                {[{key:'publicProfile',label:'Tampilkan profil fasilitas secara publik'},{key:'shareContact',label:'Bagikan kontak kepada pihak terverifikasi'},{key:'allowAnalytics',label:'Izinkan analitik penggunaan anonim'}].map(i=> (
          ...
          {activeTab==='integration' && (
            <section className="bg-white rounded-lg border border-gray-100 p-5 text-left">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="md:col-span-2"><label className="block text-sm text-gray-600 mb-1">Alamat Dompet Blockchain (opsional)</label><input className="w-full border border-gray-300 rounded-md px-3 py-2" placeholder="0x..." value={integration.walletAddress} onChange={e=>setIntegration({...integration,walletAddress:e.target.value})} /><p className="text-xs text-gray-500 mt-1">Untuk referensi penandaan transaksi on-chain.</p></div>
              </div>
              <div className="flex justify-end mt-4"><button onClick={()=>saveSettings('integration')} className="px-4 py-2 rounded-md bg-primary text-white hover:bg-primaryDark">Simpan</button></div>
            </section>
          )}
          {activeTab==='data' && (
            <section className="bg-white rounded-lg border border-gray-100 p-5 text-left">
