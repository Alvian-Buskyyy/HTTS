...
// Mengadopsi penuh struktur JagalHealth (UI & logika) untuk RPH
const RphHealth = () => {
  ...
  useEffect(() => {
    try {
      ...
      const pending = stored
        .filter(c => c.healthStatus === 'perlu_periksa' || c.healthStatus === 'sakit' || c.healthStatus === 'Perlu Dicek')
        .slice(0, 10)
        .map(c => ({
          id: c.id,
  ...
  return (
    <DashboardLayout title="Pemeriksaan & Verifikasi Kesehatan" role="RPH" customSidebar={<RphSidebar /> }>
      <div className="mt-2">
        <div className="mb-6">
          <p className="text-sm text-gray-600 leading-relaxed text-center whitespace-nowrap">Kelola proses pemeriksaan kesehatan sapi yang telah diterima untuk pemotongan. Lakukan verifikasi sebelum proses lebih lanjut.</p>
        </div>
        {successMessage && (
            ...
            <table className="min-w-full table-auto divide-y divide-gray-200 text-left">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                ...
                {verificationHistory.length === 0 && (
                  <tr><td colSpan={6} className="px-5 py-4 text-left text-sm text-gray-500">Belum ada pemeriksaan dilakukan.</td></tr>
                )}
              </tbody>
            </table>
          </div>
          ...
          <div className="fixed inset-0 flex items-center justify-center z-[60] p-4 backdrop-blur-sm bg-transparent" onClick={() => setShowVerificationModal(false)}>
            <div className="bg-white rounded-lg shadow-lg w-full max-w-3xl p-6" onClick={(e) => e.stopPropagation()}>
              <div className="flex items-start justify-between mb-4">
                <div>
                  <h3 className="text-xl font-semibold text-gray-800 flex items-center gap-2"><i className="fas fa-stethoscope text-primary"></i>Pemeriksaan Sapi #{selectedCattle.id}</h3>
                  <p className="text-xs text-gray-500">Isi data pemeriksaan di bawah ini. Kolom bertanda bintang wajib diisi.</p>
                </div>
                <span className={`px-2 py-1 text-xs rounded-full ${computeStatusLabel(formData) === 'Layak Sehat' ? 'bg-green-100 text-green-700' : computeStatusLabel(formData) === 'Lengkapi Data' ? 'bg-gray-100 text-gray-700' : 'bg-yellow-100 text-yellow-700'}`}>{computeStatusLabel(formData)}</span>
              </div>
                ...
                <div className="md:col-span-2 space-y-5">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <p className={`mt-1 text-xs ${isTempNormal(formData.suhu) === null ? 'text-gray-400' : isTempNormal(formData.suhu) ? 'text-green-600' : 'text-yellow-700'}`}>Normal: {TEMP_MIN}–{TEMP_MAX} °C {isTempNormal(formData.suhu) === null ? '' : isTempNormal(formData.suhu) ? '• Suhu normal' : '• Di luar rentang normal'}</p>
                    </div>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <input list="daftar-dokter-rph" type="text" value={formData.dokter} onChange={e=>setFormData({...formData, dokter:e.target.value})} className="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary" placeholder="Nama dokter hewan" required />
                    <datalist id="daftar-dokter-rph">
                      <option value="drh. Andi" />
                      <option value="drh. Sari" />
                      <option value="drh. Bima" />
                      <option value="drh. Lestari" />
                    </datalist>
                  </div>
                </div>
