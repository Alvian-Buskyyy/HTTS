...
const PeternakSettings = () => {
  ...
  useEffect(() => {
    try {
      const saved = JSON.parse(localStorage.getItem('peternakSettings') || '{}');
  ...
  const saveSettings = (section) => {
    const existing = JSON.parse(localStorage.getItem('peternakSettings') || '{}');
    const payload = {
    ...
    localStorage.setItem('peternakSettings', JSON.stringify(payload));
    alert('Pengaturan disimpan');
  };

  const exportData = () => {
    const exportObj = {
      settings: JSON.parse(localStorage.getItem('peternakSettings') || '{}'),
    ...
    a.href = url;
    a.download = `backup_peternak_${new Date().toISOString().slice(0,10)}.json`;
    a.click();
  ...
  const clearLocalData = () => {
    ...
    localStorage.removeItem('peternakSettings');
  ...
  return (
    <DashboardLayout title="Pengaturan" role="PETERNAK">
        ...
        <main className="md:col-span-3">
          ...
          {activeTab === 'account' && (
            <section className="bg-white rounded-lg border border-gray-100 p-5">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm text-gray-600 mb-1">Nama Peternakan</label>
                  <input className="w-full border border-gray-300 rounded-md px-3 py-2" value={account.farmName} onChange={e=>setAccount({...account, farmName: e.target.value})} />
                </div>
                <div>
                  <label className="block text-sm text-gray-600 mb-1">Nama Pemilik</label>
                  <input className="w-full border border-gray-300 rounded-md px-3 py-2" value={account.ownerName} onChange={e=>setAccount({...account, ownerName: e.target.value})} />
                </div>
                <div>
            ...
            <section className="bg-white rounded-lg border border-gray-100 p-5">
              ...
              <div className="space-y-3">
                {[
                  ...
                  { key:'healthAlerts', label:'Peringatan Kesehatan'},
                  { key:'transactionUpdates', label:'Update Transaksi'},
          ...
          {activeTab === 'privacy' && (
            <section className="bg-white rounded-lg border border-gray-100 p-5">
              ...
              <div className="space-y-3">
                {[
                  { key:'publicProfile', label:'Tampilkan profil peternakan secara publik'},
                  { key:'shareContact', label:'Bagikan kontak kepada pembeli terverifikasi'},
            ...
            <section className="bg-white rounded-lg border border-gray-100 p-5">
              ...
              <div className="space-y-4">
                <label className="flex items-center justify-between p-3 border rounded-md">
                  <span className="text-gray-700">Aktifkan 2FA (kode OTP saat transaksi)</span>
...
export default PeternakSettings;
