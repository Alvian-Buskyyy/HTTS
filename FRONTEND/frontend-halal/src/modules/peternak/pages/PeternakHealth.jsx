...
const PeternakHealth = () => {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        // Dapatkan ID peternak dari localStorage
        const userStr = localStorage.getItem('user');
        const user = userStr ? JSON.parse(userStr) : {};
        const peternakId = user.entityId || user.id;

        // Ambil daftar sapi milik peternak dari backend
        let sapiMapped = [];
        if (peternakId) {
          const sapiRes = await fetch(`${API_BASE}/sapi/entity/PETERNAK/${peternakId}`, { method: 'GET', headers });
          if (sapiRes.ok) {
        ...
        // Ambil riwayat pengecekan sehat dari backend, filter hanya sapi milik peternak
        ...
        if (sapiMapped.length) {
          ...
          if (healthRes.ok) {
            ...
            verificationRecords = await Promise.all(relevantChecks.map(async (ch) => {
              ...
              // Anggap 'terverifikasi sehat' hanya jika boolean true DAN ada CID
              if (ch.boolean === true && ch.cid) {
                sapiIdsWithTrueCheck.add(ch.sapiId);
              }
              let detail = {};
              let verifiedDate = '-';
              // Ambil payload dari IPFS berdasarkan CID untuk mendapatkan detail dan timestamp
              if (ch.cid) {
                try {
              ...
              return {
                id: ch.sapiId,
                type: sapi?.type || '-',
                source: sapi?.source || 'Kandang Sendiri',
                verifiedDate,
        ...
        // Pending: sapi yang status kesehatannya belum 'sehat'.
        // Gunakan data backend jika tersedia; jika tidak, fallback ke localStorage 'cattleList'.
        let pending = [];
        if (sapiMapped.length) {
          ...
          // Fallback: baca dari localStorage bila tidak ada data sapi dari backend
          try {
            ...
            pending = stored
              ...
              .filter(c => !sapiIdsWithTrueCheck.has(c.id)); // Keluarkan yang sudah terverifikasi sehat dengan CID
          } catch (e) {
        ...
        // Fetch default ItemSehat untuk verifikasi klinis
        try {
        ...
        console.error('Gagal memuat data kesehatan ternak:', e);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const headers = { 'Content-Type': 'application/json' };
      if (token) headers['Authorization'] = `Bearer ${token}`;
      // Sertakan detail lengkap pemeriksaan agar diupload ke IPFS oleh backend
      const payload = {
    ...
    // Hanya keluarkan dari pending dan masukkan ke riwayat jika CID tersedia
    if (created?.cid) {
      const record = {
      ...
      // Tetap biarkan di pending; informasikan sedang menunggu CID
